 ...
 */
export default class THICombat extends foundry.documents.Combat {
  /** @inheritdoc */
  prepareDerivedData() {
     ...
     * @param {THICombat} combat      The combat preparing derived data.
     */
    Hooks.callAll("THI.prepareCombatData", this);
  }
   ...
   * @returns {Promise<import("./THICombatant.mjs").default>} The created Combatant.
   */
  async addPlayer() {
    const data = {
      type: "player",
    ...
    const fdObject = await foundry.applications.api.DialogV2.input({
      window: {title: "THI.Combat.AddPlayerCombatTracker"},
   ...
   * @returns {Promise<THICombat|null>}   A Promise which resolves to the created Document, or null if the dialog was
    *                                     closed.
   */
