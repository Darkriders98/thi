...
import {addItem} from "../../helpers.mjs";

/**
 * Extend the basic ItemSheet with some very simple modifications.
 */
export class THIItemSheet extends PrimarySheetMixin(ApplicationV2Mixin(foundry.applications.sheets.ItemSheetV2)) {
  /** @inheritdoc */
  ...
  static PARTS = {
    ...
    properties: {
      template: systemPath("templates/item/properties.hbs"),
      scrollable: [""],
    }
  };
  ...
  async _preparePartContext(partId, context) {
    const fieldsets = await this._getFields();
    const descriptionfields = [];
    const otherfields = [];
    fieldsets.forEach(element => {
      if (element.fieldset) {
        otherfields.push(element);
      }
      else {
        if (['description', 'summary'].includes(element.outer.field.name)) {
          descriptionfields.push(element);
        }
        else {
          otherfields.push(element);
        }
    ...
    descriptionfields.sort((a, b) => {
      if (a.outer.field.name === "description") {
        return 1;
      }
      else if (b.outer.field.name === "description") {
        return -1;
      }
    ...
    switch (partId) {
      case "description":
        context.fields = descriptionfields;
        context.tab = context.tabs[partId];
        break;
      case "properties":
        context.fields = otherfields;
        context.tab = context.tabs[partId];
        break;
    }
    return context;
  ...
  async _getFields() {
    ...
    for (const field of Object.values(systemFields ?? {})) {
      ...
      if (field instanceof foundry.data.fields.SchemaField) {
        ...
        fieldSets.push({outer: {field, formSupport: field.constructor.hasFormSupport, value: foundry.utils.getProperty(source, path)}});
      }
    }
    return fieldSets;
  }
  ...
  async #addSystemFields(fieldset, schema, source, _path = "system") {
    for (const field of Object.values(schema)) {
      ...
      if (field instanceof foundry.data.fields.SchemaField) {
        const innerFieldSet = {fieldset: true, legend: field.label, fields: []};
        this.#addSystemFields(innerFieldSet, field.fields, source, path);
  ...
  static async #addItem(event, target) {
    addItem(this.item, "system.items.");
  }
